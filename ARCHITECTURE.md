...
Agentic Resume Tailor (ART) is a DB-first resume tailoring tool. It stores your profile in SQL, exports a normalized JSON artifact, ingests bullets into ChromaDB for retrieval, and renders a tailored LaTeX PDF.
...
1) Resume Editor writes CRUD changes to the SQL database.
2) Re-ingest exports the DB to `data/my_experience.json`, then rebuilds Chroma.
3) Generate uses the latest DB snapshot and selection pipeline to render PDF + report.

...
def test_v3_loop_applies_boost_terms_to_next_iteration(tmp_path, monkeypatch) -> None:
    ...
    iteration_two = artifacts.iteration_trace[1]
    queries_used = " ".join(iteration_two.get("queries_used", []))
    assert "fastapi" in queries_used
    assert "postgresql" in queries_used

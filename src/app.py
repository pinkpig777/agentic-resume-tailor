...
API_URL = os.environ.get("ART_API_URL", "http://localhost:8000").rstrip("/")
...
def get_health_cached(api_base: str, ttl_s: float = 2.0) -> Tuple[bool, Any]:
    ...
    return st.session_state.get("_health_ok", False), st.session_state.get("_health_info", "not checked")
...
with st.sidebar:
    ...
    max_bullets = st.slider("Max bullets on page",
                            min_value=8, max_value=24, value=16, step=1)
    max_iters = st.slider("Max loop iterations",
                          min_value=1, max_value=6, value=3, step=1)
    threshold = st.slider("Stop threshold (final score)",
                          min_value=0, max_value=100, value=80, step=1)
    alpha = st.slider("Alpha (retrieval weight)", min_value=0.0,
                      max_value=1.0, value=0.7, step=0.05)
    must_weight = st.slider("Must-have weight (coverage)",
                            min_value=0.0, max_value=1.0, value=0.8, step=0.05)
    per_query_k = st.slider("per_query_k", min_value=3,
                            max_value=30, value=10, step=1)
    final_k = st.slider("final_k", min_value=10,
                        max_value=150, value=30, step=5)
    boost_weight = st.slider("Boost query weight",
...
jd_text = st.text_area("Paste the JD here", height=260,
    ...
    if st.button("Generate", type="primary", use_container_width=True, disabled=generate_disabled):
        if not jd_text.strip():
            ...
            with st.spinner("Running agent..."):
                try:
                    resp = requests.post(
                        f"{API_URL}/generate", json=payload, timeout=600)
    ...
    if not run:
        ...
        if report:
            ...
            if best:
                ...
                c2.metric("Retrieval", round(
                    float(best.get("retrieval_score", 0.0)), 3))
                c3.metric("Coverage (bullets)", round(
                    float(best.get("coverage_bullets_only", 0.0)), 3))
                c4.metric("Coverage (all+skills)",
            ...
            if iters:
                ...
                chosen = next((x for x in iters if int(
                    x.get("iteration", -1)) == int(pick)), None)
                if chosen:
        ...
        try:
            pdf = requests.get(
                f"{API_URL}{run['pdf_url']}", timeout=120).content
            st.download_button(
        ...
        try:
            rep_bytes = requests.get(
                f"{API_URL}{run['report_url']}", timeout=60).content
            st.download_button(
